...
scripts/
  ...
  cartographer-project.sh       — Resolves a directory to its real project (worktree → parent repo); CLI face of cartographer_project() in hooks/common.sh, used by skills
...
- **`project` is resolved from the git *common* dir, never from `--show-toplevel`.** Inside a worktree the toplevel basename is the throwaway worktree directory, so a session in `psychodeli-webgl-port/.claude/worktrees/confident-yalow-e1cdc6` files under that name and every record orphans the moment the worktree is pruned — ~5,700 events, unrecoverable once the directory is gone, because the repair works by asking git to resolve the recorded cwd. `cartographer_project()` in `hooks/common.sh` is the definition; `scripts/cartographer-project.sh` is its command-line face for skills, which are markdown and cannot source a shell library. Fixing this in the hooks was only half of it: the three skills that write to `session-milestones.jsonl` each carried their own copy of the old derivation for four more months, and produced milestones that contradicted themselves — project `confident-yalow-e1cdc6` beside a digest reading `{psychodeli-webgl-port: 118}`, since the digest is built from hook events. Agent control rooms (Tessera, Kangentic) put a worktree behind every task, so any new writer must call the shared resolver rather than spell a fourth copy.
- **Parse an edit summary with `scripts/edit-paths.js`; resolve it yourself.** The edit hook writes one line per edit in two shapes — `Modified: /abs/app.js` from Edit/Write, and `Modified: a.js,b.js (via bash)` when auto mode edits through `sed -i`, a heredoc, or `open(p,'w')`. Reading that back with a bare `/^Modified:\s*(.+)$/` fails three ways at once and errors on none of them: the ` (via bash)` marker rides into the key, a multi-file row becomes one fabricated filename, and the hook's detector — which reads shell *source text* and is loose by construction — puts JS property access in the panel. Measured on session `3c5f5a20`, the digest's hottest-files panel claimed 76 files touched and led with `errors.push` ×11, with `docs/HANDOFF.md` and `docs/HANDOFF.md,docs/PRODUCT.md` sitting in it as separate entries; 58% of the 2,889 candidates the hook has emitted corpus-wide are not paths. Parsing is shared. **Resolution deliberately is not**: `memory.js` serves file contents over HTTP and must confine itself to `CORPUS_ROOT`, while `session-digest.js` only names a file and would otherwise drop real edits to `~/.claude` memory files — pass the resolver that matches the risk, and report what missed (`files_unresolved`) rather than dropping it, since a candidate can miss because a real file was since deleted.
...
- **Every Explorer view names the agent that produced the row.** Roughly half this corpus is Codex (54,826 of 110,752 changelog events; 50,533 of 86,055 tool-use rows), and the Explorer shipped provider-blind for months while `/remember` and the Memory Desk handled both — `/api/sessions` folded the field away and no classic view read it, so a mixed corpus rendered as one agent with nothing reporting a problem. `explorer/server/sessions.js` owns the fold and attributes through `isResolved()`; `computeFacets` carries a `providers` dimension (`?fa=codex`); `AgentBadge` is the one palette, checked against the 8:1 floor on both surfaces it sits on. Transcript derivation is **per-provider** — the pre-port fallback could only ever build `~/.claude/projects/<encoded>/<sid>.jsonl`, which no Codex session can match, so Codex rows silently derived nothing (7 of 28 openable in a 7-day window; 10 after the port, the other 18 genuinely absent from disk). List views use `codexSessionIndex()`, one directory walk for the whole window (1,426 files, 17 ms); the per-transcript ladder with its `find` stays in `/api/transcript`, where the user asked for one specific session. A second copy of provider attribution exists client-side (`explorer/src/lib/provider.js`) because the Timeline groups raw events itself and the server module sits on a shebanged Node script — `tests/unit/carto-provider-coverage.test.js` asserts the two agree, which is the only thing keeping that pair honest.
...
- **A window is bound at the source, never applied to a ranking.** Every ladder truncates — `FUSION_DEPTH` in both engines, `max_results` in `bm25-search.awk`'s `END` block — so filtering afterwards keeps the globally best matches and only then asks which fall inside `--since`. On a six-figure corpus a day is ~1% of events, so the ladder arrives at fusion nearly empty while every stage reports success. All three legs now bound early: `scoreBM25` filters before its slice, the awk filters in **pass 2 only**, and `semanticSearch` puts a `timestamp` range clause in the Qdrant query. Two things are load-bearing. The awk's pass 1 owns `ndocs`, `avgdl` and `df` — narrowing it recomputes IDF over a handful of documents and silently reweights every surviving score, so in-window scores must stay byte-identical to an unwindowed run. And Qdrant compares RFC3339 payload strings **chronologically**, not lexicographically (verified on 1.12.1: `2026-03-17T21:21:04-07:00` is included by `gte 2026-03-18T00:00:00Z` and excluded by `lt` on the same boundary), which matters because ~2% of payloads carry non-UTC offsets — do not "simplify" it to a string compare, and do not drop the client-side `windowed()` backstop, which still catches a pool that arrives unbounded from a server without range support.
- **Project scope is a case-insensitive substring everywhere, and Qdrant equality is not it.** `projectMatcher` in `explorer/server/project-filter.js` is the one definition: a family name selects its repositories, so `psychodeli` means `psychodeli-webgl-port`. Qdrant's `match: {value}` is exact equality, so the semantic leg once scoped to a literal string that matched nothing while the keyword leg scoped to the whole family — measured at 0 semantic rows against 9,943 indexed points for `--project psychodeli`, with `/api/recall` worst hit because it performs no registry expansion at all. Resolve the spec against the project values actually present and emit a `should` of exact matches (8–13 ms over 128k resident docs, under 1% of the request budget — computed per call, because caching it buys ~1% and costs a staleness bug the first time a new project appears). Do not "fix" this by dropping the filter and post-filtering: that reintroduces the truncation starvation above. The registry is precision and convenience, not the mechanism — an unregistered prefix must still scope both ladders alike.
...
- **A file review diff is bounded by the session's commits, never by HEAD.** `reviewRange()` in `explorer/server/memory.js` picks base = last commit at or before the session's first event and head = last commit within a grace of its last event, falling to the working tree only when the session is in flight or committed nothing for that file. `sessionBounds()` folds over the whole warm corpus, not the desk window, because a 90-day view and a 1-hour view must bound the same session the same way. Boundaries are commit *times*, not authorship: a session whose first act commits a previous session's leftovers inherits them, and the response says so in `range` caveats rather than hiding it. Do not "simplify" back to `git diff HEAD` — that answers "what is uncommitted", a different question the panel is not asking.
